...
| `auth_test.go` | Nomad authentication against a dedicated **ACL-enabled** cluster: a static `--nomad-token` authenticates and an anonymous client is denied (403, counted); workload-identity token-file auth via an explicit `--nomad-token-file` and via the auto-detected `${NOMAD_SECRETS_DIR}/nomad_token`; live **token rotation** — a rotated token in the file is applied to the running client (the prior token is revoked, and the differ keeps working) |
